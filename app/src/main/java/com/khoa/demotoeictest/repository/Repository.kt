package com.khoa.demotoeictest.repository

import com.khoa.demotoeictest.db.DataRemoteAPI
import javax.inject.Inject

class Repository @Inject constructor(private val dataRemoteAPI: DataRemoteAPI) {

    suspend fun getListParts(part: String) = dataRemoteAPI.getListParts(part)

    suspend fun getListTypesData(ets: String, test: String, part: String) = dataRemoteAPI.getListTypesData(ets,test,part)

    suspend fun getListPartsData(ets: String, test: String, part: String) = dataRemoteAPI.getListPartsData(ets,test,part)

    suspend fun getListVocab() = dataRemoteAPI.getListVocab()

    suspend fun getListVocabData(type: String) = dataRemoteAPI.getListVocabData(type)

    suspend fun putFavoriteVocabData(id: String, favorite: String) = dataRemoteAPI.putFavoriteVocabData(id,favorite)

}